...
const useSocketConnection = (socketUrl: string, userId: string, role: string, chatId?: string) => {
  ...
  useEffect(() => {
    ...
    const socketInstance = io(socketUrl, {
      ...
      timeout: 10000,
      query: { userId, role, chatId }
    });

    socketInstance.on('connect', () => {
      setIsConnected(true);
      setConnectionError(null);
      socketInstance.emit('authenticate', { userId, role, chatId });
    });
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      const response = await axios.get(`${apiBaseUrl}/chats/v1/${activeChat._id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (selectedFiles.length > 0) {
        ...
        await axios.post(`${apiBaseUrl}/chats/v1/${activeChat._id}/message`, formData, {
          headers: { 
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    <h2 className="text-lg font-semibold">
                      {activeChat.subject || 'General Chat'}
                    </h2>
